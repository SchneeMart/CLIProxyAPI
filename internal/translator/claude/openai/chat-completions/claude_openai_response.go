			...
			case "text_delta":
				...
				if text := delta.Get("text"); text.Exists() {
					...
					if len(toolCalls) > 0 {
						...
						var results [][]byte
						if normalText != "" {
							t := append([]byte{}, template...)
							t, _ = sjson.SetBytes(t, "choices.0.delta.content", normalText)
							results = append(results, t)
						}
						// Tool Calls als JSON-Text im erwarteten Format ausgeben
						jsonToolCalls := MiniMaxToolCallsToJSON(toolCalls)
						t := append([]byte{}, template...)
						t, _ = sjson.SetBytes(t, "choices.0.delta.content", jsonToolCalls)
						results = append(results, t)
						return results
					}
					...
					if pending {
						...
						if normalText != "" {
							template, _ = sjson.SetBytes(template, "choices.0.delta.content", normalText)
							return [][]byte{template}
						}
						return [][]byte{}
					}
					...
					template, _ = sjson.SetBytes(template, "choices.0.delta.content", textStr)
					hasContent = true
				}
			case "thinking_delta":
	...
	case "message_delta":
		...
		if delta := root.Get("delta"); delta.Exists() {
			if stopReason := delta.Get("stop_reason"); stopReason.Exists() {
				...
				template, _ = sjson.SetBytes(template, "choices.0.finish_reason", p.FinishReason)
			}
		}
